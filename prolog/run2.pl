...
interview :-
	...
	(   Context == teamsport,
	    einzelpartnerflag(E), /* 1 entspricht einzel und partner ist 'nein' */
	    (	E == 1,
		merkeko(2)
	    )
	),
...
baum :-
	...
	frageko(schwimmen,F1),
        ja_nein(F1,Antwort),
	(   Antwort == ja, merkeko(2),!
	 ;
	    Antwort == nein, merkeko(1),!
	),
	write('nach schwimmen'),nl,
	
	frageko(einzelsport,F2),
	ja_nein(F2,Antwort),
	(   Antwort == ja, merkeko(2),
	    
	    frageko(partnersport,F3),
	    ja_nein(F3,Antwort),
	    (   Antwort == ja, merkeko(2),!
		;
	        Antwort == nein, merkeko(1),!
	    ),
	    frageko(teamsport,F4),
	    ja_nein(F4,Antwort),
	    (   Antwort == ja, merkeko(2),!
	     ;
		Antwort == nein, merkeko(1),!
	...
	;
	    ...
	    frageko(partnersport,F5),
	    ja_nein(F5,Antwort),
	    (   Antwort == ja, merkeko(2),
	    	
		frageko(teamsport,F6),
		ja_nein(F6,Antwort),
		(   Antwort == ja, merkeko(2),!
		 ;
		    Antwort == nein, merkeko(1),!
